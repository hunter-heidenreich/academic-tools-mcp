...
- **bioRxiv's API and content hosts have outages and bot protection.** Its `/details` endpoint has served empty `200` bodies for every query while the rest of the API answered; a bioRxiv/medRxiv DOI then answers from OpenAlex (`biorxiv_unavailable: true`), which carries no `published_doi` or corresponding-author fields. `www.biorxiv.org` and `www.medrxiv.org` sit behind Cloudflare, which rate-limits or challenges automated PDF fetches; when `download_pdf` is refused, fetch the file in a browser and use `import_paper`.
...
No flag is needed for bioRxiv: when bioRxiv fails *transiently* on a `10.1101/...` DOI (5xx, 429, timeout, a garbled body), all four tools and `get_papers_metadata` answer from OpenAlex for the same DOI, tagged `_source: "openalex"` plus `biorxiv_unavailable: true`. A definitive bioRxiv miss, or a local backpressure refusal, never falls back. If OpenAlex fails transiently too, the bioRxiv error carries `openalex_fallback_retryable: true`.
