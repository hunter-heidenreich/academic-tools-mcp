...
| [Wikipedia](https://www.wikipedia.org/) | Full-text article search, summaries, Wikidata QIDs | Optional email (for User-Agent) |
...
- **A Wikipedia summary is the lead extract, not the article, and English only.** It situates a concept or a person; it is not a source, and the server queries `en.wikipedia.org` alone, so a topic covered better in another language reads as thinly covered. A disambiguation page returns an empty `extract` deliberately — its own lead describes one candidate meaning and would be miscited as the article's. Cite the `permalink`, which pins the `revision` read: the plain `url` is whatever the article later becomes, and on a contested topic that is not what you read.
...
- **arXiv refuses a request with HTTP 406 and an empty body, not a "no such paper".** Its metadata API answers an identifier it will not resolve — `2301.99999`, or a typo — this way, and does the same for some field-prefixed search queries and for any request following too closely behind an earlier rejection. The server reports it as a transient failure and re-requests rather than recording the paper as absent, because the one status covers both cases: if `get_paper_metadata` returns it for an ID you believe in, wait and retry before concluding the paper is gone. The PDF and HTML hosts ride a different edge and keep serving throughout, so `download_pdf` falls back to them when only the metadata call was refused.
...
| `get_wikipedia_summary` | Title, redirect-resolved `canonical_title`, description, extract, URL, page type (`standard` / `disambiguation`), `pageid`, `revision` + `timestamp` and a `permalink` pinning them, and `wikibase_item` — the Wikidata QID that chains on to ORCID and ROR; errors with `not_found` if the page doesn't exist |
