...
|----------|-----------------|---------------|
| [OpenAlex](https://openalex.org/) | Paper metadata, authors, institutions, abstracts, topics, citations, BibTeX | Optional API key (free, raises the credit budget 10×) |
...
- **OpenCitations cannot answer for the most-cited papers at all.** Its API times out or returns HTTP 504 on works with tens of thousands of citations — measured: `10.1038/nature14539` (~70k cited) 504s after 280s on even the cheap count endpoint, well past the client's 30s timeout — so `get_paper_citations` simply errors for them. `get_paper_citations_count` and `get_paper_references_count` fall back to a tally-only endpoint and flag the result `pageable: false`, meaning the count is real but the edge list is unavailable; take the magnitude from OpenAlex's tally instead, and read a cited paper's own bibliography for its references.
...
- **A `retracted: false` from `get_paper_updates` is not a clean bill of health.** Crossref answers a DOI nobody has deposited a notice against and a sound paper identically, so it clears a citation rather than certifying one. The flag covers the whole withdrawing class (`retraction`, `partial_retraction`, `withdrawal`, `removal`); an amending notice — a correction, an erratum, an expression of concern — is reported in `updates` without setting it, so read the list rather than the boolean. Its `relations` field is `{}` for most papers — `is-preprint-of` / `has-preprint` depend on thin publisher deposit — so an absent relation is not a claim that no preprint exists.
...
| `OPENCITATIONS_ACCESS_TOKEN` | No | Free token from [opencitations.net](https://opencitations.net/accesstoken), sent in the `authorization` header. Buys no rate tier: OpenCitations issues it to count unique users and states it is not compulsory. Those counts are how the project evidences its own relevance, which is why this client sends one. A token OpenCitations rejects makes it 403 every request, so leave it blank rather than guessing — the error then names this variable. |
...
bioRxiv needs no flag: a *transient* bioRxiv failure on a `10.1101/...` DOI is answered from OpenAlex, with `_source: "openalex"` and `biorxiv_unavailable: true`. A definitive miss or local backpressure never falls back; if OpenAlex also fails transiently, the error carries `openalex_fallback_retryable: true`.
...
An **OpenAlex work ID** works the same way, and for the same reason: `search_openalex` reports an `openalex_id` on every hit where a `doi` is often absent, and OpenCitations puts one on nearly every graph row — including the ~1.4% that carry no DOI, which were previously a dead end. Spell it `W4312223440`, `openalex:`-prefixed, or as an `openalex.org` URL; a bare run under four digits needs the prefix, so a short freeform `import_paper` label keeps meaning what it did. It too is traded for the paper's DOI on first use rather than becoming a second cache key, and a work OpenAlex indexes without a DOI is refused by name rather than 404'd.
...
| `get_institution` | Name, ROR, country, type, homepage, city/region, alternative names and acronyms, parent institutions, h-index, works/citation counts. Reached from `autocomplete_openalex`, which resolves an affiliation string to an ID |
...
| `convert_paper` | Convert a paper to markdown, parse into sections. arXiv papers use arXiv's HTML first, bioRxiv/medRxiv papers bioRxiv's JATS XML — seconds, no PDF needed. Otherwise it converts the downloaded PDF (slow: tens of minutes; `PDF_CONVERT_TIMEOUT` caps it at 30 min by default). The server runs at most one conversion at a time across all callers — a second concurrent caller gets `{busy: True, retryable: True, in_progress: {...}}` immediately rather than queueing |
| `get_paper_sections` | Section index with titles, sub-heading previews, token counts, and `conversion_mode` — what produced the markdown (`full` / `fast` / `html` / `jats` / `imported`) |
...
| Tool | Description |
|------|-------------|
| `get_paper_references_count` | Survey outgoing-reference coverage across both Crossref and OpenCitations in one call — returns per-source counts so you can pick which to page through; an OpenCitations count marked `pageable: false` came from its tally-only endpoint because the edge list timed out |
...
| `get_paper_citations_count` | Survey incoming-citation coverage across OpenCitations and OpenAlex in one call — the cross-check that tells an OpenCitations zero from a genuinely uncited paper, and the only tool that still answers for a paper whose citation list is too large to fetch |
...
| Tool | Description |
|------|-------------|
| `get_paper_code` | Linked repositories (official first, then by stars, plus the total count), project pages, Hugging Face artifacts |
| `get_paper_catalog` | Tasks, methods, introduced benchmarks and frameworks, best leaderboard ranks, predecessor and successor papers, organizations, a machine-generated TL;DR, authors' Hugging Face accounts |
| `get_paper_evaluations` | Every benchmark result a paper reports (model, metrics, rank, parameter count, open or closed), paged |
| `get_pwc_task` | A task by slug, display name or ID, with paper, benchmark and evaluation counts |
...
| `search_paperswithcode` | Keyword search, optionally limited to official code or a date range |

The paper tools take an arXiv ID in any spelling; a DOI is refused without a request. `get_paper_code` and `get_paper_catalog` share one cached record, so calling both costs one request.

**Rate limits are treated as hard.** Requests go out one at a time at no more than 90% of the documented sustained rates, never using the burst allowance. Nothing is retried automatically or paged on the agent's behalf. After a 429, every Papers with Code call returns `{error, retryable, quota_exhausted, retry_after_seconds}` without a request until `Retry-After` passes.
...
**bioRxiv/medRxiv papers do the same with bioRxiv's JATS XML** (the record's `jatsxml`), recorded as `conversion_mode: "jats"`. Most tables and many equations ship as images, which JATS carries only as captions — convert the PDF when you need them.
...
.cache/
  ...
  openalex/institutions/   # OpenAlex institution objects (JSON)
...
**arXiv HTML renderings** that don't exist are negative-cached for 1h under an `html` entity, and **bioRxiv JATS files** under a `jats` entity; the rendering itself is not cached, only the markdown made from it.
...
server.py            thin entry: re-exports mcp + tools, registers the
  ...
  │
  ├── tools/         33 @mcp.tool functions, split by job
...
- **Batch where it matters.** `get_papers_metadata` collapses N parallel singletons into batched `/works?filter=doi:...|...` calls over the OpenAlex DOIs that actually miss the cache, and batched `id_list` calls over the arXiv IDs that miss, plus concurrent fan-out for bioRxiv — designed for reference-graph enrichment.
...
- **A 429 locks the provider out.** When a provider without `X-RateLimit-*` headers answers 429 with `Retry-After`, every later call to it is refused locally until that time, instead of adding requests to the cooldown. Papers with Code, whose per-IP limit your browser shares, also gets no automatic retry.
- **Burst caps with structured backpressure.** Each provider refuses to queue more than a few concurrent callers (5 for most, 3 for Papers with Code) behind its rate-limit gap. The next one gets `{error, retryable: True, backpressure: True}` immediately so the agent learns to slow down rather than waiting silently.
...
- **The rate we take follows the identity the server confirms it received.** Crossref publishes two service tiers and stamps every response with the one that served it; the client starts at the public rate regardless of configuration and widens only on that stamp, so an address that never arrived costs throughput instead of drawing a sustained 429. Every provider sends a descriptive `User-Agent` naming the project and its repository, with a contact address appended when one is set.
