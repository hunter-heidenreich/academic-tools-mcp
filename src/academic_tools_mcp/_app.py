...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "sections. Whenever download_pdf actually downloads, it drops the "
        "cached markdown + sections so the next convert_paper picks up the "
        "new bytes; markdown you supplied via import_paper survives that "
        "unless you pass force_refresh=True.\n\n"
        ...
        "record; a search_arxiv hit is already cached, a Crossref hit warms "
        "the reference tools rather than get_paper_metadata. "
        "search_cached_papers does BM25 across "
...
FOLLOW_PUBLISHED = Annotated[
    ...
    Field(
        description=(
            ...
            "explicit; a *transient* lookup failure (one OpenAlex reports as "
            "retryable — 5xx, 429, timeout) adds "
