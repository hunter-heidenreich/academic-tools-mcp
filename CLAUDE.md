...
- Manual PDF imports (`import_pdf`, `download_pdf_url`, `import_markdown`) intentionally have no BibTeX generation — the manual pipeline has no structured metadata. When the identifier is a DOI, chain into `get_paper_bibtex` (OpenAlex) for BibTeX instead.
