...
Covers ``stems``. The sanitizer is the only reason two papers cannot share a
...
class TestSafeStem:
    ...
    def test_strips_shell_metacharacters(self):
        stem = stems.safe_stem('x"$(touch pwned)`id`;rm -rf /|y')
    ...
    def test_is_injective_for_identifiers_that_used_to_collide(self):
        ...
        assert stems.safe_stem("a b") != stems.safe_stem("a_b")
        assert stems.safe_stem("some:label") != stems.safe_stem("some_label")

    def test_percent_is_not_double_encoded(self):
        assert stems.safe_stem("x%y") == "x%25y"
    ...
    def test_encoding_its_own_output_would_double_encode(self):
        ...
        assert stems.safe_stem("a%20b") == "a%2520b"

    def test_needs_stem_migration_only_for_legacy_names(self):
        assert stems._needs_stem_migration("10.1016_s1-6(03)02831-9") is True
        assert stems._needs_stem_migration("my paper") is True
        # Already-safe and already-migrated names are left alone.
        assert stems._needs_stem_migration("2301.00001") is False
        assert stems._needs_stem_migration("10.1101_2021.01.01.123") is False
        assert stems._needs_stem_migration("a%20b") is False
        ...
        assert stems._needs_stem_migration("notes~draft%202024") is False
    ...
    def test_safe_stem_output_is_never_seen_as_legacy(self):
        ...
        for canonical in [
            ...
            assert stems._needs_stem_migration(stem) is False
    ...
    def test_a_literal_percent_2f_is_not_read_as_an_encoded_slash(self):
        ...
        """
        assert stems.safe_stem("a/b") == "a_b"
        assert stems.safe_stem("a%2Fb") == "a%252Fb"
    ...
    def test_keeps_normal_doi_and_arxiv_ids(self):
        ...
        assert stems.safe_stem("10.1101/2021.01.01.123") == "10.1101_2021.01.01.123"
        assert stems.safe_stem("2301.00001v2") == "2301.00001v2"
        assert stems.safe_stem("hep-th/9901001") == "hep-th_9901001"
...
class TestMigrateLegacyStems:
    ...
    def test_renames_legacy_names(self, tmp_path):
        ...
        moved = stems.migrate_legacy_stems()
    ...
    def test_is_idempotent(self, tmp_path):
        ...
        first = stems.migrate_legacy_stems()
        second = stems.migrate_legacy_stems()
    ...
    def test_counts_every_namespace_and_both_entity_dirs(self, tmp_path):
        ...
        assert stems.migrate_legacy_stems() == 3
...
class TestSectionsKeyForStem:
    ...
    def test_agrees_with_sections_key_for_the_identifier_that_named_the_file(self):
        for canonical in ["2301.00001", "10.1016/s1-6(03)02831-9", "notes~draft 2024", "a/b"]:
            stem = stems.safe_stem(canonical)
            assert stems.sections_key_for_stem(stem) == stems.sections_key(canonical)

    def test_re_sanitizing_a_stem_would_not_agree(self):
        stem = stems.safe_stem("my paper")
        assert stems.safe_stem(stem) != stems.sections_key_for_stem(stem)
...
class TestMarkdownPathForStem:
    def test_agrees_with_markdown_path_for_the_identifier_that_named_the_file(self):
        ...
        for canonical in ["2301.00001", "10.1016/s1-6(03)02831-9", "notes~draft 2024"]:
            stem = stems.safe_stem(canonical)
            assert stems.markdown_path_for_stem("ns", stem) == stems.markdown_path("ns", canonical)
...
class TestChecksumText:
    def test_an_empty_document_still_has_a_checksum(self, tmp_path):
        ...
        empty.write_text("")
        assert markdown_checksum(empty) == stems.checksum_text("")
