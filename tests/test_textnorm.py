"""Tests for the Unicode folding helpers (``textnorm``).
...
class TestFold:
    def test_strips_common_diacritics(self):
        assert textnorm.fold("café") == "cafe"
        assert textnorm.fold("naïve") == "naive"
        assert textnorm.fold("Gutiérrez") == "Gutierrez"

    def test_ligature_expands(self):
        # U+FB01 LATIN SMALL LIGATURE FI → "fi"
        assert textnorm.fold("ﬁ") == "fi"
    ...
    def test_empty_and_lone_combining_mark(self):
        assert textnorm.fold("") == ""
        # A lone combining acute accent folds to nothing.
        assert textnorm.fold("́") == ""

    def test_ascii_unchanged(self):
        assert textnorm.fold("transformer") == "transformer"


class TestFoldWithMap:
    def test_map_length_and_end_sentinel(self):
        folded, index_map = textnorm.fold_with_map("café")
        assert folded == "cafe"
    ...
    def test_offset_round_trip(self):
        text = "a café b"
        folded, index_map = textnorm.fold_with_map(text)
    ...
    def test_partial_expansion_span_covers_the_whole_original_char(self):
        ...
        text = "aﬁb"
        _, index_map = textnorm.fold_with_map(text)
        assert index_map[1] == index_map[2] == 1  # both halves of "fi"
        lo, hi = textnorm.original_span(index_map, 1, 2)
        assert text[lo:hi] == "ﬁ"
    ...
    def test_span_over_whole_chars_is_unchanged(self):
        ...
        folded, index_map = textnorm.fold_with_map(text)
        start = folded.index("cafe")
        lo, hi = textnorm.original_span(index_map, start, start + 4)
    ...
    def test_span_keeps_a_trailing_combining_mark(self):
        ...
        lo, hi = textnorm.original_span(index_map, 0, 4)
    ...
    def test_empty_span_stays_empty(self):
        _, index_map = textnorm.fold_with_map("abc")
        assert textnorm.original_span(index_map, 2, 2) == (2, 2)

    def test_leading_combining_mark_skipped(self):
        folded, index_map = textnorm.fold_with_map("́ab")
        assert folded == "ab"
        assert index_map[0] == 1  # first folded char comes from original idx 1

    def test_empty_input(self):
        assert textnorm.fold_with_map("") == ("", [0])

    def test_all_combining_input(self):
        assert textnorm.fold_with_map("́̀") == ("", [2])
    ...
    def test_trailing_combining_mark(self):
        ...
        folded, index_map = textnorm.fold_with_map("ab́")
        assert folded == "ab"
...
class TestFoldEquivalence:
    ...
    def test_folded_strings_agree(self, text):
        assert textnorm.fold(text) == textnorm.fold_with_map(text)[0]
...
class TestOffsetMapProperties:
    ...
    def test_map_length_matches_transform(self, text):
        for transformed, index_map in (
            textnorm.fold_with_map(text),
            textnorm.lower_with_map(text),
            textnorm.lower_with_map(text, fold=True),
        ):
    ...
    def test_any_span_maps_to_a_covering_original_slice(self, text, start, length):
        ...
        transformed, index_map = textnorm.lower_with_map(text, fold=True)
        if not transformed:
            return
        start %= len(transformed)
        end = min(len(transformed), start + 1 + length % len(transformed))
        lo, hi = textnorm.original_span(index_map, start, end)
    ...
    def test_output_equals_whole_string_transform(self, text):
        ...
        vocabulary."""
        assert textnorm.fold_with_map(text)[0] == textnorm.fold(text)
        assert textnorm.lower_with_map(text)[0] == text.lower()
        assert textnorm.lower_with_map(text, fold=True)[0] == textnorm.fold(text).lower()
...
class TestLowerWithMap:
    ...
    def test_basic_lowercase_round_trip(self):
        text = "Hello WORLD"
        lowered, index_map = textnorm.lower_with_map(text)
    ...
    def test_fold_and_lower_together(self):
        text = "CafÉ"
        lowered, index_map = textnorm.lower_with_map(text, fold=True)
        assert lowered == "cafe"
    ...
    def test_compatibility_expansion_maps_to_one_index(self):
        ...
        lowered, index_map = textnorm.lower_with_map("½a", fold=True)
        assert lowered == "1\u20442a"
        assert index_map == [0, 0, 0, 1, 2]

    def test_empty_input(self):
        assert textnorm.lower_with_map("") == ("", [0])
        assert textnorm.lower_with_map("", fold=True) == ("", [0])
    ...
    def test_equivalence_with_whole_string_transforms(self, text):
        ...
        assert textnorm.lower_with_map(text, fold=True)[0] == textnorm.fold(text).lower()
        assert textnorm.lower_with_map(text, fold=False)[0] == text.lower()
