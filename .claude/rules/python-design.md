...
- **`httpx` lives in the shared layer and the HTTP clients, nowhere else.** Only `net/clients.py` / `net/http.py` / `net/throttle.py` / `download/streaming.py`, `providers/*.py` (two providers don't need to) and `download/openaccess.py` may import it — the last four for their `_get_client() -> httpx.AsyncClient` annotation. A tool, pipeline or content module that does is a layering violation: `tools/pipeline.py` drives the whole download → convert → sections pipeline through `openaccess` and the providers with no `import httpx`.
