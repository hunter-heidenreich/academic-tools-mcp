...
- **A local write failure is an `{error, retryable: True}`, never a raised `OSError`.** `cache.put` already refuses to let an ENOSPC escape an MCP tool; the PDF write path owes the same contract, and `retryable` keeps a full disk out of the negative cache where it would strand the paper. `stream_to_file`'s `except OSError` is that guarantee — a new disk operation added inside it inherits it, one added outside does not.
