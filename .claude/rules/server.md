...
- **Every error a pipeline tool returns carries a `suggestion`.** `_http`'s vocabulary is a retry verdict, not advice, and the providers add none, so the tool layer is where a download failure learns that `import_paper` exists. `_enrich_error` fills a gap and never overwrites — a provider that shipped its own advice (`oa_download`) keeps it.
...
**That tag reads `_http`'s verdict, never the absence of another key.** The error vocabulary is three-state — `retryable: True`, `not_found: True`, and an unclassified 4xx carrying neither — so `retryable is True` is the only test that means "a retry might work". Inverting `not_found` collapses three states into two and sends an agent back at a call that cannot succeed. `oa_download._resolve_and_download` holds the same rule from the other side.
