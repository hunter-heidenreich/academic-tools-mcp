...
- **Citation keys are ASCII `[a-z0-9]`.** `_key_token` is the only gate: `_TRANSLIT` handles the characters `_textnorm.fold` can't decompose, then fold + lowercase + strip. A new key component must route through `_key_token`, as `_extract_last_name` and `_first_key_word` do.
- **`_escape_bibtex` treats field text as literal and `_escape_doi` does not** — a DOI must stay resolvable, so braces are escaped rather than stripped. Both are single-pass, never chained `str.replace`: escaping `\` first would emit braces a later brace-pass re-escapes.
