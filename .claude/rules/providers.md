...
- `_MAX_CONCURRENT` / `_MIN_REQUEST_GAP` / `_MAX_PENDING` are declared per provider with the reason in a comment beside them, and passed to a module-level `Throttle` exposed through thin `_throttled_get` (→ `_throttle.get`) and, in the PDF-downloading modules, `_request_slot` (→ `_throttle.slot`) wrappers. crossref's are **resolved at import from config**, not literals — never read them as fixed numbers.
- Module-level `_single_flight` instance, passed to `cache.cached_lookup`.
- Each getter is `canonical = canonical_*(id)` → an async `_fetch()` closure holding the HTTP + parse + caching decisions → `return await cache.cached_lookup(...)`. `cached_lookup` owns force_refresh invalidation, the outer and in-slot re-checks, single-flight coalescing, and the per-caller deep copy. Accept `force_refresh: bool = False` and thread it in; don't hand-roll any of the rest.
- Pass a tuple `sf_key` when one canonical id has multiple sub-fetches (openalex `("work"|"author", canonical)`, opencitations `("references"|"citations", canonical)`, the PDF paths `("pdf", canonical)`); omit it to key on `canonical`.
...
- **Identifiers reaching a request path are percent-encoded** via `quote(...)`, so reserved characters (`#`, `?`, a stray `/`) can't split the path or truncate the request to the wrong record. `safe=` differs per provider — see below. **Two paths are unquoted and should be**: `biorxiv._fetch`'s details URL and `acl_anthology.pdf_url`.

## openalex.py

Singleton endpoints (`/works/{id}`, `/authors/{id}`). ID normalization for DOI formats, OpenAlex URLs, ORCIDs. Each entity has a private `_normalize_*` and a **public** `canonical_*` (`canonical_doi`, `canonical_author_id`) — `manual` and the tool layer import the canonical form.

**DOI handling.** `_normalize_doi` strips surrounding whitespace and both `https://`/`http://doi.org/` (and `doi:`) prefixes; the bare DOI is encoded into `/works/doi:{doi}` with `quote(..., safe="/")`. Wrong shape here means non-dict *or* missing the entity `id` key. `get_author`'s 404 carries `not_found: True` to match `get_work`.

**Batch fetch:** `get_works_batch(dois, *, force_refresh=False)` collapses N cache-miss DOIs into ⌈N/50⌉ HTTP calls via `/works?filter=doi:DOI1|DOI2|...`. Cached entries (positive or negative) are served without a network call. Each fetched work is written to the singleton cache, so a follow-up `get_work(doi)` is a free hit. A DOI containing OpenAlex filter metacharacters (`|` = OR, `,` = AND) would corrupt the OR-joined filter, so it is split out and resolved individually through `get_work`. A parse failure or non-dict body on the batch GET maps every DOI in that chunk to a retryable `_parse_error_dict()`; non-dict items inside `results` are skipped.
...
Per-chunk errors are built one fresh dict per key (`dict.fromkeys` would alias one object across 50). `_fetch_chunk` deliberately does **not** deep-copy its single-flight result, unlike `cached_lookup` — per-key freshness removes the aliasing hazard and the objects are read-only to every consumer; the rationale is in its docstring.
...
**`retry_attempts=3`, not the shared default.** arXiv's Fastly edge returns 429/503 with no `Retry-After` when an IP is briefly penalty-boxed, and one retry tends to land in the same cooldown; two ride `get_with_retry`'s backoff out of it.
...
`_collection_of` is what makes that safe: it returns `None` for a wrong-shape body and a list (possibly empty) for a well-formed one. Only a well-formed empty collection is "not found" and negative-cached; both-garbled is `_parse_error_dict()`. `AttributeError`/`TypeError` are in neither `_PARSE_ERRORS` nor `HTTPX_ERRORS`, so an unguarded body escapes the provider entirely.

Selects the latest version from multi-version responses. `published_doi` links to the journal DOI when available — `get_paper_metadata(..., follow_published=True)` auto-chains to OpenAlex. No documented upstream limit, so the pacing is deliberately conservative.
...
`get_work` takes `force_refresh` so the reference-graph tools can refresh both sources — the reference list grows as publishers re-deposit metadata.
...
If you touch these constants, keep the two halves in lockstep: **the rate we take must follow the identity we send.** Hardcoding the polite tier while the mailto stays unconfigured means the documented default (an empty `.env`) would request at 2× the public-pool rate, 3× its concurrency and 3× its search rate.
...
OpenCitations Index API v2. Outgoing references (`/references/doi:...`) and incoming citations (`/citations/doi:...`). Parses space-delimited multi-ID strings (`omid:... doi:... openalex:... pmid:...`) via `_parse_ids()`.
...
MediaWiki OpenSearch (`/w/api.php?action=opensearch`) for title search; Wikimedia REST (`/api/rest_v1/page/summary/{title}`) for summaries and existence verification. Detects disambiguation pages via the `type` field. **Requests without a `User-Agent` may be blocked outright.**
