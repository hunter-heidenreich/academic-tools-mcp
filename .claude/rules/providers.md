...
- **An anomalous 200 of the wrong shape is treated identically**, rather than crashing the parse. Each provider knows what "wrong shape" means for its endpoint (non-dict, missing the entity `id`, not a list of records), and none of them is ever positive-cached for the TTL. **The guard goes all the way down to the elements**: a list-shaped payload is checked for being a list *and* its entries for being dicts, because `item.get(...)` raises `AttributeError` — in neither `_PARSE_ERRORS` nor `HTTPX_ERRORS` — on a non-dict entry. A wrong shape is an error, never an empty result set: reported as "no matches", it ends the agent's search instead of prompting a retry.
