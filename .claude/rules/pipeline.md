...
`papers.store_markdown_and_index()` is the only place a sections-cache entry is assembled, and every writer routes through it — both conversion modes via `_finalize_markdown()`, and `manual.import_markdown` directly. **Invariant: an entry always carries all four of `sections`, `sections_detected`, `markdown_checksum`, `conversion_mode`.** `import_markdown` assembled its own with two of them, and since `_reparse_sections_locked` accepts an entry whose checksum matches, the missing `sections_detected` was never recomputed — `get_paper_sections` reported an imported heading-free paper as having detected sections, the exact reading `sections_note` exists to prevent.
...
- **The gap is per host, and that is the whole point.** It was 0.0, justified as "every URL is a different host" — an assumption, not a fact. The URLs come from OpenAlex, and a reference walk through one journal resolves many DOIs to the same publisher domain, which then got fetched back-to-back with no pacing at all. `_MIN_REQUEST_GAP` is now 1 req/s/host. `max_concurrent` stays **global**: it bounds our own egress (sockets, fds, and simultaneous in-flight streams, since `stream_to_file` holds the slot for the whole download), so making it per-host would let a 20-publisher walk open 40 parallel streams.
...
- **Only definitive failures are negative-cached** (24h, entity `downloads`), so a retrying agent doesn't re-resolve OpenAlex and re-fetch the same non-PDF every call. The predicate is `_pdf_download.is_definitive_failure`, now shared by all four download paths, and it is an **allowlist** (`retryable is False`). It was a denylist local to this module (`retryable is not True`), which looked equivalent and was not: `_http.error_dict` tags only `LocalBackpressureError` with `retryable`, so a timeout, a 5xx and a 429 all arrived with no such key and were cached as permanent for 24h. A `MAX_PDF_BYTES` abort stays excluded despite being non-retryable — a config choice a cap bump fixes, not a fact about the paper.
