...
- **`pdf_path(namespace, canonical)` is the sibling of `markdown_path`, and every provider delegates to it.** Each keeps only its own canonicalization step. A local `safe_stem(x) + ".pdf"` is a fifth copy of a rule that has one home. A caller holding a *stem* off disk rather than an identifier takes `markdown_path_for_stem` / `sections_key_for_stem` — re-sanitizing a stored stem encodes its own escapes. Call it as `_stems.pdf_path` — it is deliberately *not* re-exported through `papers`, where the name would shadow each provider's own one-argument `pdf_path(identifier)`.
...
- **The sweep may not raise.** It runs inside `_lifespan`, so a directory it cannot read must be skipped, not propagated — an unreadable `.cache/` subtree would otherwise stop the server from starting. `_list_dir` is that gate, and it also materialises the listing, because the sweep renames files into the directory it is walking (`manual.migrate_misrouted_arxiv` does the same, for the same reason).
...
The on-disk half of that comparison is a *test* oracle — `tests/_checksums.py`, deliberately not importable from `src/`. Re-adding it to `_stems` so a writer can reach it is the bug the paragraph above describes.
