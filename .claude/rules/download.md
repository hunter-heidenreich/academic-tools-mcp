...
- **The temp file is created only once the response is worth writing** — after the 404 / `raise_for_status` / Content-Type gates, via `NamedTemporaryFile` (which binds the fd to the file object, so there is no raw-fd leak to bookkeep). A response rejected at one of those gates therefore touches no disk at all, not even `dest.parent`; only the `%PDF-` magic-byte rejection, which reads the first chunk, creates and then unlinks a temp file.
...
- **`MAX_PDF_BYTES`** is resolved by `resolve_max_pdf_bytes()`, a thin call to `config.number` — the disable vocabulary lives there (`_DISABLE_VALUES`) and only that vocabulary disables. The `on_nonpositive="default"` argument is the policy this module owns: a *negative* value falls back to the default rather than reading as an "unlimited" idiom, because silently dropping the disk guard is the opposite of what a mistyped cap should do. The two `PDF_*_TIMEOUT`s pass `"disable"`; don't copy one policy onto the other. A download that would exceed the cap is aborted *during* the stream, so a misrouted URL can't fill the disk before any size check. The abort is `written + len(chunk) > max_bytes`: a PDF of exactly `MAX_PDF_BYTES` lands.
...
Throttling is `per_host=True` with a global `max_concurrent`; the mechanism and the reason `max_concurrent` must not go per-host are in `.claude/rules/net.md` § `per_host`, and the constants carry their own justification in this module.
